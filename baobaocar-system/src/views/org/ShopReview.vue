  ...
  <section>
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true" :model="queryData">
        ...
        <el-form-item>
          <el-button type="primary" v-on:click="getshopOperateLogs">查询</el-button>
        </el-form-item>
        <el-form-item>
    ...
    <el-table
      ...
      <el-table-column prop="operateId" label="操作人id" width="180" sortable>
      </el-table-column>

      <el-table-column prop="operateTime" label="操作时间" width="180" sortable>
      </el-table-column>

      <el-table-column prop="operateType" label="状态" width="180" sortable>
        <template slot-scope="scope">
          <span v-if="scope.row.operateType === 1" class="status-pending">待审核</span>
          <span v-else-if="scope.row.operateType === 2" class="status-approved">审核通过</span>
          <span v-else-if="scope.row.operateType === 3" class="status-success">激活成功</span>
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      this.getshopOperateLogs();
    },
    //获取用户列表
    getshopOperateLogs() {
      // 获取参数 - currentPage pageSize keyword
      // 发起请求
      this.$http.post("/shopOperateLog",this.queryData)
          .then(res =>{
            res=res.data
            console.log(res)
            if (res.success){
    ...
    deletebyId(id){
      // 发起请求
      this.$http.delete("/shopOperateLog/"+id)
          .then(res =>{
            res = res.data
            if (res.success){
    ...
    saveOrUpdate(){
      ...
      this.$http.put("/shopOperateLog",this.saveForm)
          .then(res =>{
            res = res.data
            if (res.success){
    ...
    batchDelete(){
      // 发起请求
      this.$http.patch("/shopOperateLog",this.sels)
          .then(res =>{
            res = res.data
            if (res.success){
              this.$message({ message: '删除成功', type: 'success' });
